...
public class SimpleTransitionSystem extends TransitionSystem{
    ...
    public boolean isDeterministicHelper() {
        ...
        while (!waiting.isEmpty()) {
            ...
            toStore.extrapolateMaxBounds(this.getMaxBounds(),clocks.getClocks());
            passed.add(toStore);
            ...
            for (Channel action : actions) {
                ...
                toAdd.forEach(e->e.extrapolateMaxBounds(getMaxBounds(),clocks.getClocks()));
    ...
    public boolean checkConsistency(State currState, Set<Channel> inputs, Set<Channel> outputs, boolean canPrune) {
        ...
        toStore.extrapolateMaxBounds(getMaxBounds(),clocks.getClocks());
        passed.add(toStore);
    ...
    public boolean isImplementationHelper(){
        ...
        while (!waiting.isEmpty()) {
            ...
            for (Channel action : actions){
                ...
                toAdd.forEach(s -> s.extrapolateMaxBounds(getMaxBounds(),clocks.getClocks()));
    ...
    private boolean passedContainsState(State state1) {
       State state = new State(state1);
        state.extrapolateMaxBounds(maxBounds, clocks.getClocks());
