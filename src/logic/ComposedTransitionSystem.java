				...
				if (outputsOutside.contains(channel)) {
						for (int i = 0; i < locations.size(); i++) {
								...
								for (Transition transition : transitions) {
										...
										if (isDbmValid(dbm)) {
												State newState = new State(newLocations, dbm);
												StateTransition stateTransition = new StateTransition(currentState, newState, channel, false, transition.getGuards(), transition.getUpdates());
						...
						for (int n = 0; n < newLocationsArr.size(); n++) {
								...
								if (isDbmValid(dbm)) {
										State newState = new State(newLocations, dbm);
										StateTransition stateTransition = new StateTransition(currentState, newState, channel, true, guards, updates);
										stateTransitions.add(stateTransition);
								}
						}
				}
