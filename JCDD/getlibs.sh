...
mkdir -p "$SOURCE_DIR/libs/sources/UDBM-2.0.11/build"
cd "$SOURCE_DIR/libs/sources/UDBM-2.0.11"
cd build
cmake $CMAKE_ARGS -DCMAKE_INSTALL_PREFIX="$SOURCE_DIR/libs/UDBM" ..
cmake --build . --config Release
cmake --install . --config Release
...
mkdir -p "$SOURCE_DIR/libs/sources/UCDD-0.2.1/build"
cd "$SOURCE_DIR/libs/sources/UCDD-0.2.1"
cd build
cmake $CMAKE_ARGS -DCMAKE_INSTALL_PREFIX="$SOURCE_DIR/libs/UCDD" ..
cmake --build . --config Release
cmake --install . --config Release
